...
const providerSource = fs.readFileSync(path.join(repoRoot, "src/providers/provider-registry.js"), "utf8");
assert.match(providerSource, /google:[\s\S]*eventsStorageKey: "calendarClockEvents"[\s\S]*sourceStorageKey: "calendarClockSource"/);
assert.match(providerSource, /outlook:[\s\S]*eventsStorageKey: "calendarClockOutlookEvents"[\s\S]*sourceStorageKey: "calendarClockOutlookSource"/);
assert.match(popupSource, /createPopupProvider\("google"/);
assert.match(popupSource, /createPopupProvider\("outlook"/);
assert.match(popupSource, /storage\.local\.get\(\[provider\.eventsStorageKey, provider\.sourceStorageKey\][\s\S]*runtime\?\.lastError/);
assert.match(popupSource, /calendarClockPopupProvider/);
